...
if __name__ == '__main__':
    ...
    for f in files:
        # 多目标
        g_args = copy.deepcopy(args)
        g_args['file-name'] = f
        prcs = Process(target=main, args=(copy.deepcopy(g_args),))
        process_pool.append(prcs)

        # 单目标
        g_args['multi'] = False
        g_args['selection'] = 'single'
        prcs = Process(target=main, args=(copy.deepcopy(g_args),))
        process_pool.append(prcs)

    for prcs in process_pool:
        prcs.start()
        # in order not to mix output
        time.sleep(10)
    for prcs in process_pool:
        prcs.join()
